...
<p>Your personal health record — helps us tailor analysis, precautions, and doctor matches</p>
    ...
    with st.form("health_profile_form"):
        ...
        st.markdown("""<div class="section-header" style="margin-top:1.2rem;">Known Conditions</div>""", unsafe_allow_html=True)
        ...
        st.markdown("""<div class="section-header" style="margin-top:1.2rem;">Current Medications</div>""", unsafe_allow_html=True)
    ...
    if existing:
        ...
        st.markdown("""<div class="section-header">Current Profile Summary</div>""", unsafe_allow_html=True)
...
<div style="font-size:0.68rem; color:#475569; text-transform:uppercase; letter-spacing:0.8px; margin-top:0.2rem;">{label}</div>
        ...
        if existing.get("conditions"):
            ...
            conditions_html = " ".join([
                f"<span style='background:rgba(239,68,68,0.06); border:1px solid rgba(239,68,68,0.15); border-radius:20px; padding:0.25rem 0.7rem; font-size:0.78rem; color:#fca5a5; margin:0.15rem; display:inline-block;'>{c}</span>"
                for c in existing["conditions"]
            ])
            st.markdown(f"<div><b style='color:#64748b; font-size:0.85rem;'>Known Conditions:</b><div style='margin-top:0.4rem;'>{conditions_html}</div></div>", unsafe_allow_html=True)
