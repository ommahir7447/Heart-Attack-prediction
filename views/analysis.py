...
def render_analysis():
    st.markdown("""<div style="background:rgba(30,41,59,0.5); border:1px solid rgba(148,163,184,0.08); border-radius:12px; padding:1.5rem 2rem; margin-bottom:1.5rem;"><h1 style="font-family:'Sora',sans-serif; font-size:1.6rem; font-weight:700; margin:0; color:#e2e8f0;">Deep Health Analysis</h1><p style="color:#64748b; font-size:0.85rem; margin:0.15rem 0 0;">Plain-English breakdown of each clinical metric — what it means, why it matters, what to do</p></div>""", unsafe_allow_html=True)
        ...
        with st.expander(f"{metric}  —  {val} {info['unit']}  {badge}", expanded=False):
            ...
            with col1:
                ...
                fig.add_annotation(x=val, y=0.5, text=f"<b>You: {val}</b>",
                                   showarrow=False, font=dict(color="#38bdf8", size=12))
                fig.update_layout(
                ...
                <div style="background:rgba(30,41,59,0.5); border:1px solid rgba(148,163,184,0.08);
                    ...
                    <b style="color:#38bdf8;">What to do:</b> {info['tips']}
                </div>
                """, unsafe_allow_html=True)
