...
METRIC_INFO = {
    ...
    "Cholesterol": {
        "unit": "mg/dl", "normal_range": (150, 200),
...
def risk_badge(val, warning, danger):
    if val >= danger:
        return "<span style='background:rgba(239,68,68,0.08); border:1px solid rgba(239,68,68,0.2); border-radius:20px; padding:0.2rem 0.6rem; font-size:0.75rem; color:#ef4444;'>High Risk</span>"
    elif val >= warning:
        return "<span style='background:rgba(251,191,36,0.08); border:1px solid rgba(251,191,36,0.2); border-radius:20px; padding:0.2rem 0.6rem; font-size:0.75rem; color:#fbbf24;'>Borderline</span>"
    else:
        return "<span style='background:rgba(52,211,153,0.08); border:1px solid rgba(52,211,153,0.2); border-radius:20px; padding:0.2rem 0.6rem; font-size:0.75rem; color:#34d399;'>Normal</span>"

def render_analysis():
    st.markdown("""<div class="page-header"><h1>Deep Health Analysis</h1><p>Plain-English breakdown of each clinical metric — what it means, why it matters, what to do</p></div>""", unsafe_allow_html=True)
    ...
    risk_color = "#ef4444" if risk_pct >= 60 else "#fbbf24" if risk_pct >= 30 else "#34d399"
    risk_label = "High Risk" if risk_pct >= 60 else "Moderate Risk" if risk_pct >= 30 else "Low Risk"
    st.markdown(f"""
    <div class="glass-card" style="display:flex; align-items:center; gap:1.2rem; margin-bottom:1.2rem;">
        ...
        <div>
            ...
            <div style="color:#475569; font-size:0.82rem; margin-top:0.15rem;">
    ...
    for metric, info in METRIC_INFO.items():
        ...
        with st.expander(f"{metric}  —  {val} {info['unit']}  {badge}", expanded=False):
            col1, col2 = st.columns([2, 1.2])
            with col1:
                fig = go.Figure()
                fig.add_trace(go.Bar(x=[info["danger"] * 1.5], y=[metric], orientation='h',
                    marker_color='rgba(239,68,68,0.08)', showlegend=False))
                fig.add_trace(go.Bar(x=[info["warning"]], y=[metric], orientation='h',
                    marker_color='rgba(251,191,36,0.08)', showlegend=False))
                fig.add_trace(go.Bar(x=[info["normal_range"][1]], y=[metric], orientation='h',
                    marker_color='rgba(52,211,153,0.1)', showlegend=False))
                ...
                st.markdown(f"""
                <div class="glass-card" style="font-size:0.85rem; color:#64748b; line-height:1.7; margin-top:0.4rem;">
                    <b style="color:#e2e8f0;">What this means:</b><br>{info['explanation']}<br><br>
                    <b style="color:#34d399;">What to do:</b> {info['tips']}
                </div>
                """, unsafe_allow_html=True)
            ...
            with col2:
                ...
                <div class="glass-card" style="font-size:0.82rem; line-height:1.8;">
                    <div style="color:#334155; font-size:0.7rem; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:0.4rem;">Reference</div>
                    <div>Normal: <b style="color:#34d399;">{normal_range_text}</b></div>
                    <div>Borderline: <b style="color:#fbbf24;">{info['warning']} {info['unit']}</b></div>
                    <div>High Risk: <b style="color:#ef4444;">{info['danger']}+ {info['unit']}</b></div>
                    <div>Your Value: <b style="color:#e2e8f0;">{val} {info['unit']}</b></div>
                    <hr style="border-color:rgba(148,163,184,0.05); margin:0.6rem 0;">
                    <div style="color:#334155; font-size:0.7rem; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:0.4rem;">Watch for symptoms</div>
                    {"".join([f"<div style='color:#64748b;'>• {s}</div>" for s in info['symptoms']])}
                </div>
                """, unsafe_allow_html=True)
