...
def render_doctors():
    st.markdown("""<div class="page-header"><h1>Find Doctors</h1><p>Connect with cardiologists and specialists — request an appointment instantly</p></div>""", unsafe_allow_html=True)
    ...
    st.markdown(f"<p style='color:#475569; margin:0.6rem 0; font-size:0.85rem;'>Found <b style='color:#e2e8f0;'>{len(doctors)}</b> doctors matching your criteria</p>", unsafe_allow_html=True)

    if not doctors:
        st.markdown("""
        <div style="text-align:center; padding:3rem; background:rgba(12,20,42,0.5);
             border:1px dashed rgba(148,163,184,0.1); border-radius:16px;">
            <div style="font-size:1.5rem; color:#334155;">—</div>
            <div style="color:#e2e8f0; font-weight:600; margin-top:0.5rem;">No doctors found</div>
            <div style="color:#475569; font-size:0.85rem;">Try changing your filters</div>
        </div>
    ...
    for i, doc in enumerate(doctors):
        ...
        avail_badge = (
            "<span style='background:rgba(52,211,153,0.08); border:1px solid rgba(52,211,153,0.2); border-radius:20px; padding:0.2rem 0.6rem; font-size:0.72rem; color:#34d399;'>● Available</span>"
            if doc["available"] else
            "<span style='background:rgba(100,116,139,0.06); border:1px solid rgba(100,116,139,0.15); border-radius:20px; padding:0.2rem 0.6rem; font-size:0.72rem; color:#475569;'>○ Booking Only</span>"
        )
        specialty_color = {
            "Cardiologist": "#a78bfa", "Cardiac Surgeon": "#ef4444", "General Physician": "#34d399"
        }.get(doc["specialty"], "#64748b")
        ...
        <div class="glass-card" style="margin-bottom:0.8rem;">
            <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:0.7rem;">
                <div>
                    ...
                    <div style="color:#475569; font-size:0.78rem; margin-top:0.1rem;">{doc['hospital']} · {doc['city']}</div>
                </div>
                {avail_badge}
            </div>
            <div style="display:flex; gap:1.2rem; font-size:0.8rem; color:#64748b; margin-bottom:0.6rem;">
                <span>Rating: <b style="color:#fbbf24;">{doc['rating']}</b>/5.0</span>
                <span>{doc['experience']} yrs exp</span>
                <span>{doc['fee']}</span>
            </div>
            <div style="font-size:0.78rem; color:#475569;">Ph: {doc['phone']}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <div style="background:linear-gradient(135deg, rgba(52,211,153,0.04), rgba(139,92,246,0.03));
         border:1px solid rgba(52,211,153,0.08);
         border-radius:10px; padding:0.8rem 1.1rem; font-size:0.8rem; color:#6ee7b7;">
